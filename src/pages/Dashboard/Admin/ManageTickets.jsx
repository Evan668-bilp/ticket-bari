...
const ManageTickets = () => {
  ...
  useEffect(() => {
    axios.get(`${import.meta.env.VITE_API_URL}/api/tickets/all`, { headers: { authorization: `Bearer ${localStorage.getItem('token')}` } })
      .then(res => setTickets(res.data));
  }, []);

  const handleApprove = (id) => {
    axios.patch(`${import.meta.env.VITE_API_URL}/api/tickets/${id}/approve`, {}, { headers: { authorization: `Bearer ${localStorage.getItem('token')}` } })
      .then(() => setTickets(tickets.map(t => t._id === id ? { ...t, status: 'approved' } : t)));
  };

  const handleReject = (id) => {
    axios.patch(`${import.meta.env.VITE_API_URL}/api/tickets/${id}/reject`, {}, { headers: { authorization: `Bearer ${localStorage.getItem('token')}` } })
