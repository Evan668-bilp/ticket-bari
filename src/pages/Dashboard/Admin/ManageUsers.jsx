...
const ManageUsers = () => {
  ...
  const handleMakeAdmin = (id) => {
    axios.patch(`${import.meta.env.VITE_API_URL}/api/users/${id}/role`, { role: 'admin' }, { headers: { authorization: `Bearer ${localStorage.getItem('token')}` } })
      .then(() => setUsers(users.map(u => u._id === id ? { ...u, role: 'admin' } : u)));
  };

  const handleMakeVendor = (id) => {
    axios.patch(`${import.meta.env.VITE_API_URL}/api/users/${id}/role`, { role: 'vendor' }, { headers: { authorization: `Bearer ${localStorage.getItem('token')}` } })
      .then(() => setUsers(users.map(u => u._id === id ? { ...u, role: 'vendor' } : u)));
  };

  const handleMarkFraud = (id) => {
    axios.patch(`${import.meta.env.VITE_API_URL}/api/users/${id}/fraud`, {}, { headers: { authorization: `Bearer ${localStorage.getItem('token')}` } })
